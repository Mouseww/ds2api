...
| WebUI 管理台 | `/admin` 单页应用（中英文双语、深色模式，支持查看服务器端对话记录）；`/admin` 默认落地页为 Dashboard 用量仪表盘（此前为 Accounts），显式路由如 `/admin/accounts` 不受影响 |
| 管理后台 Dashboard | 可视化 Token 用量、请求次数、错误数、延迟，以及实时 RPM/TPM 与历史趋势图（`1h`/`24h`/`7d`/`30d`/`90d`/`1y`，数据来自 `GET /admin/usage-stats`） |
| 账号用量监测 | Dashboard 新增「账号用量」视图：按账号维度统计所选区间的请求次数、成功率、Token 用量、流式请求与平均 RPM/TPM，支持排行榜与单账号趋势下钻（`GET /admin/usage-stats/accounts`、`GET /admin/usage-stats/accounts/{identifier}`）；在获取到账号前即失败的请求单独提示，不摊派给真实账号 |
...
```bash
# 1. （可选）准备环境变量和配置文件
#    缺少 .env 时所有环境变量使用下方 docker-compose.yml 中的默认值；
#    缺少 config.json 时服务会自动以空配置启动并打印告警。
# cp .env.example .env
# cp config.example.json config.json

# 2. （可选）编辑 .env（至少设置 DS2API_ADMIN_KEY；如需修改宿主机端口，可额外设置 DS2API_HOST_PORT）
...
镜像会预创建 `/data` 并授权给非 root 的 `ds2api` 用户。
**注意**：若宿主机 `config.json` 不存在，Docker 会把它创建为**空目录**而非文件（单文件 bind mount 缺源时的行为）。此时服务仍会启动，但日志会输出 `WARN` 告警，指导移除该目录并创建配置文件。建议在首次启动前执行 `cp config.example.json config.json` 以避免目录陷阱。如使用单文件 bind mount，还需确保 `config.json` 对容器用户可读写，例如 `chmod 644 config.json`。
**语义边界**：只有「配置不可用」才会自举启动——文件不存在、配置路径是目录、配置文件对容器用户不可读（权限不足）；「配置内容错误」仍然致命并保持退出——JSON 非法或 `ValidateConfig` 语义校验失败。
...
- `current_input_file`：全局生效的上下文拆分上传策略；默认关闭，开启且阈值为 `0` 时，触发时将完整上下文合并上传为 `DS2API_HISTORY.txt` 上下文文件。
...
- `DS2API_USAGE_STATS_PATH`（环境变量）：管理台 Dashboard 用量统计的持久化文件；默认 `data/usage_stats.json`，在 Vercel 上且未设置该变量时回落到 `/tmp/usage_stats.json`（运行时 `/var/task` 只读）。具体语义以 `internal/config/paths.go` 的 `UsageStatsPath()` 为准。
...
- **构建产物**：多平台二进制包（`linux/amd64`、`darwin/amd64`、`windows/amd64`）、Linux Docker 镜像导出包 + `sha256sums.txt`
